...
async function main(){
    ...
    const UFIBuyerDemo = await deployer.loadArtifact("UFIBuyerDemo");
    const TestTokenFaucet = await deployer.loadArtifact("TestTokenFaucet");
    ...
    console.log("Subscriptions version: ", (await subscriptionsContract.version()).toString());
    ...
    const verifier = new Contract(VERIFIER_ADDRESS, PureFiVerifier.abi, wallet);
    const PARAM_TYPE1_DEFAULT_COUNTRYKYC_RULE = 7;     // ruleId = 131
    const PARAM_TYPE1_DEFAULT_AGEKYC_RULE = 8;         // ruleId = 231
    const PARAM_TYPE1_DEFAULT_COUNTRYAGEKYC_RULE = 9;  // ruleId = 331
    ​
    const PARAM_TYPE2_DEFAULT_AML_RULE = 24;
    const PARAM_TYPE2_DEFAULT_KYC_RULE = 25;
    const PARAM_TYPE2_DEFAULT_KYCAML_RULE = 26;
    await verifier.setUint256(PARAM_TYPE1_DEFAULT_COUNTRYKYC_RULE, 131);
    await verifier.setUint256(PARAM_TYPE1_DEFAULT_AGEKYC_RULE, 231);
    await verifier.setUint256(PARAM_TYPE1_DEFAULT_COUNTRYAGEKYC_RULE, 331);
    console.log('changed verifier');
    //adding test contract for the demo front-end


    const demo_contract = await deployer.deploy(UFIBuyerDemo);

    console.log("demo_contract address :", demo_contract.address);
    await new Promise(resolve => setTimeout(resolve, 3000)); // 3 sec
    await (await demo_contract.initialize(UFI_ADDRESS, VERIFIER_ADDRESS)).wait();

    const ufiToken = new Contract(UFI_ADDRESS, TestTokenFaucet.abi, wallet);
    await(await ufiToken.transfer(demo_contract.address,BigNumber.from(1000000).mul(decimals))).wait();
