...
export default async function (hre: HardhatRuntimeEnvironment) {
    ...
    // await swapTokens(paymasterContract, wallet, deployer);
...
async function delistToken(token: string, paymaster: ethers.Contract, wallet: Wallet) {
    ...
    const whitelistedTokens = await paymaster.connect(wallet).getWhitelistedTokens();

    const isWhitelisted = await paymaster.connect(wallet).isWhitelisted(token);

    if (!isWhitelisted && !whitelistedTokens.includes(token)) {
...
// in this case, only with mock plugin check for approving whitelisted tokens 
async function swapTokens(paymaster: ethers.Contract, wallet: Wallet, deployer: Deployer) {
    const erc20Artifact = await deployer.loadArtifact("MyERC20");
    const usdcContract = new ethers.Contract(usdc, erc20Artifact.abi);
    const usdtContract = new ethers.Contract(usdt, erc20Artifact.abi);

    // first mint tokens for paymaster that will be "swapped";
    await (await usdcContract.connect(wallet).mint(paymaster.address, ethers.BigNumber.from("10000000"))).wait();
    await (await usdtContract.connect(wallet).mint(paymaster.address, ethers.BigNumber.from("10000000"))).wait();

    const pluginAllowanceUSDCBefore = await usdcContract.connect(wallet).allowance(paymaster.address, mockPlugin);
    const pluginAllowanceUSDTBefore = await usdtContract.connect(wallet).allowance(paymaster.address, mockPlugin);

    const receivedETH = await (await paymaster.connect(wallet).swapTokens()).wait();

    const pluginAllowanceUSDCAfter = await usdcContract.connect(wallet).allowance(paymaster.address, mockPlugin);
    const pluginAllowanceUSDTAfter = await usdtContract.connect(wallet).allowance(paymaster.address, mockPlugin);

    pluginAllowanceUSDCAfter
        .sub(pluginAllowanceUSDCBefore)
        .eq(ethers.BigNumber.from(10000000)) ? console.log("Success approve (USDC) ") : console.log("Approve fail (USDC)");

    pluginAllowanceUSDTAfter
        .sub(pluginAllowanceUSDTBefore)
        .eq(ethers.BigNumber.from(10000000)) ? console.log("Success approve (USDT) ") : console.log("Approve fail (USDT)");
...
const getMinTokenAllowance = async (hre: HardhatRuntimeEnvironment, requiredETH: ethers.BigNumberish) => {
    ...
    const requiredAllowance = await mockPluginContract.connect(wallet).getMinTokensAmountForETH(usdc, requiredETH);
