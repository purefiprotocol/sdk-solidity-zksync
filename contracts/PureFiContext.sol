...
abstract contract PureFiContext is Initializable {
    ...
    uint256 internal constant _PUREFI_CONTEXT_NOT_VERIFIED =
    ...
    function __PureFiContext_init_unchained(
    ...
    modifier rejectUnverified() {
        require(
            _txLocalPureFiCheckResult == _PUREFI_CONTEXT_VERIFIED,
            "PureFiContext : context not set or unverified"
        );
        _;
    }
    ...
    modifier withDefaultAddressVerification(
        DefaultRule _rule,
        address _address,
        ...
        if (_rule == DefaultRule.AML)
            ruleID = IParamStorage(pureFiVerifier).getUint256(
                PARAM_TYPE1_DEFAULT_AML_RULE
            );
        else if (_rule == DefaultRule.KYC)
            ruleID = IParamStorage(pureFiVerifier).getUint256(
                PARAM_TYPE1_DEFAULT_KYC_RULE
            );
        else if (_rule == DefaultRule.KYCAML)
            ruleID = IParamStorage(pureFiVerifier).getUint256(
                PARAM_TYPE1_DEFAULT_KYCAML_RULE
            );
        else if (_rule == DefaultRule.COUNTRYKYC)
            ruleID = IParamStorage(pureFiVerifier).getUint256(
                PARAM_TYPE1_DEFAULT_COUNTRYKYC_RULE
            );
        else if (_rule == DefaultRule.AGEKYC)
            ruleID = IParamStorage(pureFiVerifier).getUint256(
                PARAM_TYPE1_DEFAULT_AGEKYC_RULE
            );
        else if (_rule == DefaultRule.COUNTRYAGEKYC)
            ruleID = IParamStorage(pureFiVerifier).getUint256(
                PARAM_TYPE1_DEFAULT_COUNTRYAGEKYC_RULE
            );
        else if (_rule == DefaultRule.Type2KYCAML)
            ruleID = IParamStorage(pureFiVerifier).getUint256(
                PARAM_TYPE2_DEFAULT_KYCAML_RULE
            );
        else require(false, "PureFiContext : Incorrect rule provided");
        _verifyAgainstTheRuleType1(ruleID, _address);
        //here the smart contract can decide whether to fail a transaction in case of check failed
        _;
        _clearContext();
    }
    modifier withCustomAddressVerification(
        uint256 _ruleID,
        address _address,
        bytes calldata _purefidata
    ) {
    ...
    function getVerificationPackage()
        internal
        view
        returns (VerificationPackage memory)
    {
        require(
            _txLocalPureFiCheckResult == _PUREFI_CONTEXT_VERIFIED,
            "PureFiContext : Verification failed"
        );
        return
            IPureFiVerifier(pureFiVerifier).decodePureFiPackage(
                _txLocalPureFiPackage
            );
    }
    function _verifyAgainstTheRuleType1(
        uint256 _ruleID,
        address _address
    ) private view {
        VerificationPackage memory package = IPureFiVerifier(pureFiVerifier)
            .decodePureFiPackage(_txLocalPureFiPackage);
        require(
            package.rule == _ruleID,
            "PureFiContext : package rule mismatch"
        );
        require(
            package.from == _address,
            "PureFiContext : package address mismatch"
        );
    }
    ...
    function _validateAndSetContext(bytes calldata _purefidata) private {
        ...
        (purefiPackage, _txLocalPureFiCheckResult) = IPureFiVerifier(
            pureFiVerifier
        ).validatePureFiData(_purefidata);
        require(
            _txLocalPureFiCheckResult == _PUREFI_CONTEXT_VERIFIED,
            "PureFiContext : Verification failed"
        );
