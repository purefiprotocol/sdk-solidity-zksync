...
contract FilteredPool is Initializable {
    ...
    function toggleTestMode() external {
        //for testing purposes only. Should be onlyOwner
        testMode = !testMode;
    }

    function setPureFiContext(address _purefiPaymaster) external {
        //test testing purposes only. Should be onlyOwner
        contextHolder = IPureFiTxContext(_purefiPaymaster);
    }
     ...
     */
    function withdrawTo(uint256 _amount, address _to) external {
        //verify sender funds via PureFiContext
        (
            uint256 sessionID,
            uint256 ruleID,
        ...
        if (!testMode) {
            require(
                ruleID == expectedWithdrawRuleID,
                "Invalid ruleID provided"
            );
            require(
                msg.sender == verifiedUser,
                "Invalid verifiedUser provided"
    ...
    function _deposit(uint256 amount, address to) private {
        require(
            basicToken.transferFrom(msg.sender, address(this), amount),
